...
/usr/local/bin/aws autoscaling describe-auto-scaling-instances --region $region   --query "AutoScalingInstances[?AutoScalingGroupName=='$AutoScalingGroupName']" --output text | cut -f4 > mediawiki-oldInstances.txt
...
do
  echo "Retry Number $i"
  /usr/local/bin/aws elbv2 describe-target-health --target-group-arn $targetGroup --region $region  > mediawiki-elbdata
...
if [ $success != 1 ]; then
  ...
  /usr/local/bin/aws autoscaling update-auto-scaling-group --region $region  --auto-scaling-group-name $AutoScalingGroupName --termination-policies NewestInstance
  /usr/local/bin/aws autoscaling update-auto-scaling-group --region $region  --auto-scaling-group-name $AutoScalingGroupName --min-size $originalCapacity
  /usr/local/bin/aws autoscaling set-desired-capacity --region $region  --auto-scaling-group-name $AutoScalingGroupName --desired-capacity $originalCapacity
  sleep 90
  /usr/local/bin/aws autoscaling update-auto-scaling-group --region $region  --auto-scaling-group-name $AutoScalingGroupName --termination-policies OldestInstance
  exit 1;
fi


/usr/local/bin/aws autoscaling update-auto-scaling-group --region $region  --auto-scaling-group-name $AutoScalingGroupName --min-size $finalCapacity
/usr/local/bin/aws autoscaling set-desired-capacity --region $region  --auto-scaling-group-name $AutoScalingGroupName --desired-capacity $finalCapacity
...
do
  for (( j=1; j <= $retry; j++ ))
  do
    if [ $(/usr/local/bin/aws ec2 describe-instances --instance-id $instance  --region $region --query "Reservations[].Instances[].State[].Name" --output text) == "terminated" ]; then
